    ...
    for (size_t i = 0; i < input_len; i += 4) {
        ...
        for (int j = 0; j < 4; ++j) {
            ...
            if (pos != std::string::npos)
                block = (block << 6) | static_cast<int>(pos);
        }
...
unsigned char* base64_decode(const char* data, size_t input_length, size_t* output_length) {
    ...
    *output_length = input_length / 4 * 3;
    ...
    for (size_t i = 0, j = 0; i < input_length;) {
        int sextet_a = data[i] == '=' ? 0 & i++ : strchr((const char*)b64_table, data[i++]) - (const char*)b64_table;
        int sextet_b = data[i] == '=' ? 0 & i++ : strchr((const char*)b64_table, data[i++]) - (const char*)b64_table;
        int sextet_c = data[i] == '=' ? 0 & i++ : strchr((const char*)b64_table, data[i++]) - (const char*)b64_table;
        int sextet_d = data[i] == '=' ? 0 & i++ : strchr((const char*)b64_table, data[i++]) - (const char*)b64_table;
