...
class ProposedAlgorithm:
    ...
    def get_unix_time(self):
        """Get current unix timestamp from system time"""
    ...
    def PPE_with_separate_timing(self, inp, salt):
        """Main encryption function with separate key generation timing"""
        key = base64.b64encode(self.get_current_time_key(salt).encode()).decode()
        return self.multi_core_encrypt(inp, key), key
    
    def PPD_with_separate_timing(self, inp, salt, key=None):
        """Main decryption function with separate key generation timing"""
        if key is None:
            key = base64.b64encode(self.get_current_time_key(salt).encode()).decode()
...
def benchmark_algorithms():
    ...
    for i in range(iterations):
        ...
        key_start_time = time.perf_counter_ns()
        key = base64.b64encode(proposed.get_current_time_key("reza").encode()).decode()
        key_gen_time = (time.perf_counter_ns() - key_start_time) / 1_000_000
        key_gen_times.append(key_gen_time)
        
        # Warm up run
        if i == 0:
            _ = proposed.multi_core_encrypt(test_data[:100], key)
        
        # Measure encryption time (after key generation)
        start_time = time.perf_counter_ns()
        enc_proposed = proposed.multi_core_encrypt(test_data, key)
        encrypt_time = (time.perf_counter_ns() - start_time) / 1_000_000
        encrypt_times.append(encrypt_time)
        
        # Measure decryption time (using same key)
        start_time = time.perf_counter_ns()
        dec_proposed = proposed.multi_core_decrypt(enc_proposed, key)
    ...
    print(f"Proposed - Key Gen: {avg_key_gen_time:.4f}ms, Encrypt: {results['Proposed']['encrypt_time']:.4f}ms, Decrypt: {results['Proposed']['decrypt_time']:.4f}ms")
    print(f"Proposed - Total (without key gen): {results['Proposed']['total_time']:.4f}ms")
...
def create_comparison_chart(results):
    ...
    ax.set_title('Encryption Algorithms Performance Comparison\n(Key Generation Time Excluded for Proposed Algorithm)', fontsize=16, fontweight='bold')
    ax.set_xticks(x)
...
if __name__ == "__main__":
    ...
    # Display results
    print("\n" + "=" * 50)
    print("BENCHMARK RESULTS (Encryption/Decryption Only)")
    print("=" * 50)
    ...
    for alg_name, times in benchmark_results.items():
        ...
        if alg_name == 'Proposed':
            print(f"  Note: Key generation time excluded from measurements")
