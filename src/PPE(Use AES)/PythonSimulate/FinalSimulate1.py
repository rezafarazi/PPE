...
class AlgorithmSimulator:
    ...
    def simulate_chacha20_performance(self, data_size_bytes):
        ...
        # ChaCha20 is typically faster than AES but slower than PPE
        ...
        # Memory calculation
        base_memory = 2.5 + random.uniform(-0.2, 0.3)  # Smaller than RSA, larger than AES
        key_memory = 0.032  # 32-byte key
        total_memory = base_memory + key_memory
        
        # Power calculation (single core but efficient)
...
def create_dynamic_comparison_data():
    ...
    return comparison_data, ppe_result, chacha20_result, aes_result, rsa_result

def create_dynamic_performance_metrics(ppe_result, chacha20_result, aes_result, rsa_result):
    ...
    metrics = {
        ...
        'ChaCha20 (Single-Thread)': [
            chacha20_result['time_ms'], 
            chacha20_result['memory_kb'], 
            chacha20_result['power_mw'], 
            8, 8, 6, 
            chacha20_result['thread_efficiency'], 
            7
        ],
        'AES (Single-Thread)': [
            aes_result['time_ms'], 
...
def print_dynamic_comprehensive_table():
    ...
    print("=" * 150)
    print("ESP32 COMPREHENSIVE ALGORITHM COMPARISON - DYNAMIC SIMULATION RESULTS")
    print("=" * 150)
    ...
    print("RSA: Simulated single-thread asymmetric encryption with heavy computation")
    print("=" * 150)
    
    data, ppe_result, chacha20_result, aes_result, rsa_result = create_dynamic_comparison_data()
    ...
    return ppe_result, chacha20_result, aes_result, rsa_result
...
def create_individual_charts(simulation_results):
    ...
    bars2 = plt.bar(x - 0.5*width, core_data['ChaCha20'], width, label='ChaCha20 (Single-Thread)', color=colors[1], alpha=0.8, edgecolor='black')
    bars3 = plt.bar(x + 0.5*width, core_data['AES'], width, label='AES (Single-Thread)', color=colors[2], alpha=0.8, edgecolor='black')
    bars4 = plt.bar(x + 1.5*width, core_data['RSA'], width, label='RSA (Single-Thread)', color=colors[3], alpha=0.8, edgecolor='black')
    ...
    plt.plot(data_sizes, chacha20_times, 's-', label='ChaCha20 (Stream Cipher)', color=colors[1], linewidth=3, markersize=8)
    plt.plot(data_sizes, aes_times, '^-', label='AES (Block Cipher)', color=colors[2], linewidth=3, markersize=8)
    plt.plot(data_sizes, rsa_times, 'd-', label='RSA (Asymmetric)', color=colors[3], linewidth=3, markersize=8)
    ...
    plt.plot(data_sizes, aes_memory, '^-', label='AES (Standard Implementation)', color=colors[2], linewidth=3, markersize=8)
    plt.plot(data_sizes, rsa_memory, 'd-', label='RSA (Large Key Storage)', color=colors[3], linewidth=3, markersize=8)
    ...
    plt.plot(data_sizes, chacha20_power, 's-', label='ChaCha20 (Single-Core Efficient)', color=colors[1], linewidth=3, markersize=8)
    plt.plot(data_sizes, aes_power, '^-', label='AES (Single-Core Intensive)', color=colors[2], linewidth=3, markersize=8)
    plt.plot(data_sizes, rsa_power, 'd-', label='RSA (Computationally Heavy)', color=colors[3], linewidth=3, markersize=8)
    ...
    chacha20_consistency = [simulator.simulate_chacha20_performance(64)['time_ms'] for _ in range(runs)]
    ...
    plt.plot(time_points, chacha20_consistency, 's-', label='ChaCha20 (Stream Consistency)', color=colors[1], linewidth=3, markersize=8)
    plt.plot(time_points, aes_consistency, '^-', label='AES (Block Stability)', color=colors[2], linewidth=3, markersize=8)
    plt.plot(time_points, rsa_consistency, 'd-', label='RSA (Variable Performance)', color=colors[3], linewidth=3, markersize=8)
    ...
    chacha20_efficiency = [1000 / (time * power) for time, power in zip(chacha20_times, chacha20_power)]
    ...
    plt.plot(data_sizes, chacha20_efficiency, 's-', label='ChaCha20 (Good Efficiency)', color=colors[1], linewidth=3, markersize=8)
    plt.plot(data_sizes, aes_efficiency, '^-', label='AES (Moderate Efficiency)', color=colors[2], linewidth=3, markersize=8)
    plt.plot(data_sizes, rsa_efficiency, 'd-', label='RSA (Low Efficiency)', color=colors[3], linewidth=3, markersize=8)
    ...
    chacha20_base_scores = calculate_efficiency_score(
    ...
    chacha20_scores = chacha20_base_scores + [chacha20_base_scores[0]]
    ...
    ax.plot(angles, chacha20_scores, 's-', linewidth=3, label='ChaCha20 (Single-Thread)', color=colors[1], markersize=8)
    ax.fill(angles, chacha20_scores, alpha=0.25, color=colors[1])
    
    ax.plot(angles, aes_scores, '^-', linewidth=3, label='AES (Single-Thread)', color=colors[2], markersize=8)
    ax.fill(angles, aes_scores, alpha=0.25, color=colors[2])
    
    ax.plot(angles, rsa_scores, 'd-', linewidth=3, label='RSA (Single-Thread)', color=colors[3], markersize=8)
    ax.fill(angles, rsa_scores, alpha=0.25, color=colors[3])
    
    ax.set_xticks(angles[:-1])
    ax.set_xticklabels(categories, fontsize=12)
    ax.set_ylim(0, 10)
    ax.set_title('Overall Algorithm Performance Radar (Dynamic Simulation Results)', 
...
def create_dynamic_visualization(simulation_results):
    ...
    print("=" * 80)
    print("Each chart will be displayed in a separate window")
...
def print_simulation_analysis():
    ...
    print("• ChaCha20 Performance: Stream cipher with lightweight single-thread processing")
...
def main():
    ...
    print("RSA: Single-thread asymmetric encryption simulation")
    print("=" * 80)
    ...
    ppe_result, chacha20_result, aes_result, rsa_result = print_dynamic_comprehensive_table()
    ...
    print(f"ChaCha20 Speed: {chacha20_result['time_ms']:.1f}ms with stream cipher efficiency")
    print(f"ChaCha20 Memory: {chacha20_result['memory_kb']:.1f}KB lightweight implementation")
    print(f"ChaCha20 Power: {chacha20_result['power_mw']:.0f}mW single-core efficient")
    ...
    print("ChaCha20: Excellent choice for embedded systems requiring good security/performance balance")
    print("AES: Best for applications prioritizing maximum security over speed")
    print("RSA: Use only for key exchange, avoid for bulk data encryption")
    print("Hybrid Approach: PPE + RSA combination for optimal security/performance")
    print("Stream vs Block: ChaCha20 offers better embedded performance than AES")
