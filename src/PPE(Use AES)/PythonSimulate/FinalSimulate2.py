...
class AlgorithmSimulator:
    ...
    def simulate_chacha20_performance(self, data_size_bytes):
        """Simulate ChaCha20 single-thread performance"""
        
        # ChaCha20 is a fast stream cipher
        base_time_per_byte = 0.3 + random.uniform(-0.05, 0.05)
        
        # Single thread processing
        total_time = data_size_bytes * base_time_per_byte
        
        # Memory calculation
        base_memory = 2.5 + random.uniform(-0.3, 0.3)
        key_memory = 0.5  # 256-bit key + nonce
        total_memory = base_memory + key_memory
        
        # Power calculation (efficient)
        processing_power = 80 + (data_size_bytes / 100) * 2
        total_power = self.base_power_consumption + processing_power
        ...
        return {
            ...
            'memory_kb': total_memory,
            'power_mw': total_power,
            'core_utilization': [65 + random.uniform(-5, 10), 5 + random.uniform(0, 5)],
...
def create_dynamic_comparison_data():
    ...
    chacha_result = simulator.simulate_chacha20_performance(64)
    ...
    comparison_data = {
        ...
        'ChaCha20 Algorithm (Single-Thread)': [
            f'{chacha_result["time_ms"]:.1f} ms (stream processing)',
            f'{chacha_result["memory_kb"]:.1f} (lightweight implementation)',
            f'{chacha_result["power_mw"]:.0f} (efficient processing)',
            'Sequential stream processing',
            f'Single-thread execution (efficiency: {chacha_result["thread_efficiency"]:.1f}/10)',
            'Excellent - lightweight for embedded',
            'High - modern stream cipher',
            '256-bit key + nonce',
            'Good - adaptable',
            'Medium - stream-based design',
            'Excellent - constant time scaling',
            f'Single-core ({chacha_result["core_utilization"][0]:.0f}% / {chacha_result["core_utilization"][1]:.0f}%)',
            'None - no multi-threading',
            'Excellent - very fast'
        ]
    }
    
    return comparison_data, ppe_result, aes_result, rsa_result, chacha_result

def create_dynamic_performance_metrics(ppe_result, aes_result, rsa_result, chacha_result):
    ...
    time_values = [ppe_result['time_ms'], aes_result['time_ms'], rsa_result['time_ms'], chacha_result['time_ms']]
    ...
    speed_scores = [
        ...
        normalize_score(chacha_result['time_ms'], min_time, max_time, inverse=True)
    ]
    
    # Memory efficiency (lower memory = higher score)
    memory_values = [ppe_result['memory_kb'], aes_result['memory_kb'], rsa_result['memory_kb'], chacha_result['memory_kb']]
    ...
    memory_scores = [
        ...
        normalize_score(chacha_result['memory_kb'], min_memory, max_memory, inverse=True)
    ]
    
    # Power efficiency (lower power = higher score)
    power_values = [ppe_result['power_mw'], aes_result['power_mw'], rsa_result['power_mw'], chacha_result['power_mw']]
    ...
    power_scores = [
        ...
        normalize_score(chacha_result['power_mw'], min_power, max_power, inverse=True)
    ]
    ...
    metrics = {
        ...
        'ChaCha20 (Single-Thread)': [
            chacha_result['time_ms'], 
            chacha_result['memory_kb'], 
            chacha_result['power_mw'], 
            8, 8, 5, 
            chacha_result['thread_efficiency'], 
            8
        ]
    }
...
def print_dynamic_comprehensive_table():
    ...
    print("🌊 ChaCha20: Simulated single-thread stream cipher implementation")
    print("=" * 140)
    
    data, ppe_result, aes_result, rsa_result, chacha_result = create_dynamic_comparison_data()
    ...
    return ppe_result, aes_result, rsa_result, chacha_result
...
def create_individual_charts(simulation_results):
    ...
    core_data = {
        ...
        'ChaCha20': chacha20_results[1]['core_utilization']
    }
    
    x = np.arange(2)
    width = 0.2
    
    bars1 = plt.bar(x - 1.5*width, core_data['PPE'], width, label='PPE (Dual-Thread)', color=colors[0], alpha=0.8, edgecolor='black')
    bars2 = plt.bar(x - 0.5*width, core_data['AES'], width, label='AES (Single-Thread)', color=colors[1], alpha=0.8, edgecolor='black')
    bars3 = plt.bar(x + 0.5*width, core_data['RSA'], width, label='RSA (Single-Thread)', color=colors[2], alpha=0.8, edgecolor='black')
    bars4 = plt.bar(x + 1.5*width, core_data['ChaCha20'], width, label='ChaCha20 (Single-Thread)', color=colors[3], alpha=0.8, edgecolor='black')
    ...
    chacha_times = [result['time_ms'] for result in chacha20_results]
    
    plt.plot(data_sizes, ppe_times, 'o-', label='PPE (Parallel Processing)', color=colors[0], linewidth=3, markersize=8)
    plt.plot(data_sizes, aes_times, 's-', label='AES (Sequential Processing)', color=colors[1], linewidth=3, markersize=8)
    plt.plot(data_sizes, rsa_times, '^-', label='RSA (Mathematical Operations)', color=colors[2], linewidth=3, markersize=8)
    plt.plot(data_sizes, chacha_times, 'd-', label='ChaCha20 (Stream Processing)', color=colors[3], linewidth=3, markersize=8)
    ...
    chacha_memory = [result['memory_kb'] for result in chacha20_results]
    
    plt.plot(data_sizes, ppe_memory, 'o-', label='PPE (with Thread Management)', color=colors[0], linewidth=3, markersize=8)
    plt.plot(data_sizes, aes_memory, 's-', label='AES (Standard Implementation)', color=colors[1], linewidth=3, markersize=8)
    plt.plot(data_sizes, rsa_memory, '^-', label='RSA (Large Key Storage)', color=colors[2], linewidth=3, markersize=8)
    plt.plot(data_sizes, chacha_memory, 'd-', label='ChaCha20 (Lightweight)', color=colors[3], linewidth=3, markersize=8)
    ...
    chacha_power = [result['power_mw'] for result in chacha20_results]
    
    plt.plot(data_sizes, ppe_power, 'o-', label='PPE (Dual-Core Efficient)', color=colors[0], linewidth=3, markersize=8)
    plt.plot(data_sizes, aes_power, 's-', label='AES (Single-Core Intensive)', color=colors[1], linewidth=3, markersize=8)
    plt.plot(data_sizes, rsa_power, '^-', label='RSA (Computationally Heavy)', color=colors[2], linewidth=3, markersize=8)
    plt.plot(data_sizes, chacha_power, 'd-', label='ChaCha20 (Efficient Stream)', color=colors[3], linewidth=3, markersize=8)
    ...
    chacha_consistency = [simulator.simulate_chacha20_performance(64)['time_ms'] for _ in range(runs)]
    
    time_points = list(range(1, runs + 1))
    
    plt.plot(time_points, ppe_consistency, 'o-', label='PPE (Parallel Consistency)', color=colors[0], linewidth=3, markersize=8)
    plt.plot(time_points, aes_consistency, 's-', label='AES (Sequential Stability)', color=colors[1], linewidth=3, markersize=8)
    plt.plot(time_points, rsa_consistency, '^-', label='RSA (Variable Performance)', color=colors[2], linewidth=3, markersize=8)
    plt.plot(time_points, chacha_consistency, 'd-', label='ChaCha20 (Fast Consistency)', color=colors[3], linewidth=3, markersize=8)
    ...
    chacha_efficiency = [1000 / (time * power) for time, power in zip(chacha_times, chacha_power)]
    
    plt.plot(data_sizes, ppe_efficiency, 'o-', label='PPE (High Efficiency)', color=colors[0], linewidth=3, markersize=8)
    plt.plot(data_sizes, aes_efficiency, 's-', label='AES (Moderate Efficiency)', color=colors[1], linewidth=3, markersize=8)
    plt.plot(data_sizes, rsa_efficiency, '^-', label='RSA (Low Efficiency)', color=colors[2], linewidth=3, markersize=8)
    plt.plot(data_sizes, chacha_efficiency, 'd-', label='ChaCha20 (High Efficiency)', color=colors[3], linewidth=3, markersize=8)
    ...
    chacha_base_scores = calculate_efficiency_score(
        chacha20_results[base_result_64]['time_ms'],
        chacha20_results[base_result_64]['memory_kb'],
        chacha20_results[base_result_64]['power_mw'],
        chacha20_results[base_result_64]['thread_efficiency']
    )
    chacha_scores = chacha_base_scores + [chacha_base_scores[0]]
    
    ax.plot(angles, ppe_scores, 'o-', linewidth=3, label='PPE (Dual-Thread)', color=colors[0], markersize=8)
    ax.fill(angles, ppe_scores, alpha=0.25, color=colors[0])
    
    ax.plot(angles, aes_scores, 's-', linewidth=3, label='AES (Single-Thread)', color=colors[1], markersize=8)
    ax.fill(angles, aes_scores, alpha=0.25, color=colors[1])
    
    ax.plot(angles, rsa_scores, '^-', linewidth=3, label='RSA (Single-Thread)', color=colors[2], markersize=8)
    ax.fill(angles, rsa_scores, alpha=0.25, color=colors[2])
    
    ax.plot(angles, chacha_scores, 'd-', linewidth=3, label='ChaCha20 (Single-Thread)', color=colors[3], markersize=8)
    ax.fill(angles, chacha_scores, alpha=0.25, color=colors[3])
...
def print_simulation_analysis():
    ...
    print("• ChaCha20 Performance: Stream cipher with constant-time processing")
...
def main():
    ...
    print("🌊 ChaCha20: Single-thread stream cipher simulation")
    print("=" * 80)
    ...
    ppe_result, aes_result, rsa_result, chacha_result = print_dynamic_comprehensive_table()
    ...
    print(f"⚡ ChaCha20 Speed: {chacha_result['time_ms']:.1f}ms with {chacha_result['thread_efficiency']:.1f}/10 thread efficiency")
    print(f"💾 ChaCha20 Memory: {chacha_result['memory_kb']:.1f}KB lightweight")
    print(f"🔋 ChaCha20 Power: {chacha_result['power_mw']:.0f}mW efficient")
    print(f"🧵 Core Usage: {chacha_result['core_utilization'][0]:.0f}% / {chacha_result['core_utilization'][1]:.0f}% (Core 0/Core 1)")
    ...
    print("🌊 ChaCha20: Excellent for high-speed, lightweight encryption requirements")
