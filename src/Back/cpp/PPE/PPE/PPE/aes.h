...
uint32_t GETU32(const uint8_t* plaintext) {
	return ((uint32_t)(plaintext[0]) << 24) |
		((uint32_t)(plaintext[1]) << 16) |
		((uint32_t)(plaintext[2]) << 8) |
		((uint32_t)(plaintext[3]));
}

void PUTU32(uint8_t* ciphertext, uint32_t value) {
	ciphertext[0] = (uint8_t)(value >> 24);
	ciphertext[1] = (uint8_t)(value >> 16);
	ciphertext[2] = (uint8_t)(value >> 8);
	ciphertext[3] = (uint8_t)value;
}
...
static void rijndaelEncrypt(const uint32_t* rk, int32_t nrounds, const uint8_t plaintext[AES_BLOCK_SIZE], uint8_t ciphertext[AES_BLOCK_SIZE])
{
...
static void rijndaelDecrypt(const uint32_t* rk, int32_t nrounds, const uint8_t ciphertext[AES_BLOCK_SIZE], uint8_t plaintext[AES_BLOCK_SIZE]) {
	uint32_t s0, s1, s2, s3, t0, t1, t2, t3;
...
void AES_ecb_encrypt(const uint8_t* in, uint8_t* out, const uint8_t* key, uint32_t keybits) {
	if (keybits != 128 && keybits != 192 && keybits != 256) {
		return;
	}

	uint32_t roundKeys[60] = { 0 };
	int32_t numRounds;

	numRounds = rijndaelSetupEncrypt(roundKeys, key, keybits);

	rijndaelEncrypt(roundKeys, numRounds, in, out);
}

void AES_ecb_decrypt(const uint8_t* in, uint8_t* out, const uint8_t* key, uint32_t keybits) {
	if (keybits != 128 && keybits != 192 && keybits != 256) {
		return;
	}
	...
	numRounds = rijndaelSetupDecrypt(roundKeys, key, keybits);

	rijndaelDecrypt(roundKeys, numRounds, in, out);
}
