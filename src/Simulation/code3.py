...
class ESP32Simulator:
    ...
    def simulate_cpu_slowdown(self, operation_time):
        ...
        if operation_time > 0:
            # Calculate delay time for slowdown simulation
    ...
    def simulate_memory_fragmentation(self):
        ...
        # ESP32 often has fragmentation issues
        time.sleep(0.001)  # Small delay for allocation
        gc.collect()  # garbage collection like ESP32
...
class ESP32CryptoSimulator:
    ...
    def get_current_time_key(self, salt):
        ...
        # Simulate slow ESP32 operations
        unix_time = str(int(time.time()))
        timestep = int(unix_time)
        timestep8 = str(timestep)[:8]
        timestep8char = list(timestep8)
        
        # Less iterations for ESP32 memory limitations
        keybase = timestep8
        for i in range(4):  # Less than 8 in original code
            keybase += ''.join(reversed(timestep8char))
            keybase += ''.join(timestep8char)
            
            # Simulate ESP32 slowdown
    ...
    def ppe_encrypt(self, data, key):
        """PPE encryption like ESP32"""
        start_time = time.perf_counter()
        
        # Check memory limitations
        data_bytes = data.encode('utf-8')
        self.simulator.check_memory_limit(len(data_bytes) * 3)  # Estimate memory usage
        
        key_bytes = key[:16]
        result = bytearray()
        
        # Simulate byte-by-byte ESP32 processing
        for i, byte in enumerate(data_bytes):
            result.append(byte ^ key_bytes[i % len(key_bytes)])
            
            # Every 100 bytes a small delay (simulate ESP32 limitations)
            if i % 100 == 0:
                self.simulator.simulate_cpu_slowdown(0.0001)
    ...
    def ppe_decrypt(self, hex_data, key):
        """PPE decryption"""
        start_time = time.perf_counter()
    ...
    def aes_encrypt(self, data, key):
        ...
        try:
            ...
            for i in range(0, len(padded_data), chunk_size):
                ...
                # Simulate ESP32 slowdown for AES
    ...
    def parallel_aes_encrypt(self, data, key):
        """Parallel AES encryption - ESP32 optimized version"""
        start_time = time.perf_counter()
        
        try:
            # Check memory limitations
            self.simulator.check_memory_limit(len(data) * 5)  # Parallel processing needs more memory
            
            # Use our ParallelAESCrypto class
            crypto = ParallelAESCrypto(key.decode('utf-8'))
            encrypted = crypto.encrypt(data)
            
            operation_time = time.perf_counter() - start_time
            self.simulator.simulate_cpu_slowdown(operation_time * 1.5)  # Parallel might be faster
            
            return encrypted
            
        except Exception as e:
            return f"Parallel AES encryption failed: {str(e)}"
    
    def parallel_aes_decrypt(self, encrypted_data, key):
        """Parallel AES decryption - ESP32 optimized version"""
        start_time = time.perf_counter()
        
        try:
            # Check memory limitations
            self.simulator.check_memory_limit(len(encrypted_data) * 4)
            ...
            return f"Parallel AES decryption failed: {str(e)}"
    ...
    def rsa_encrypt(self, data, public_key):
        ...
        try:
            ...
            for i in range(0, len(data_bytes), max_chunk_size):
                ...
                # Simulate severe ESP32 slowdown for RSA
...
def esp32_algorithm_benchmark():
    """Compare PPE, AES, Parallel AES, and RSA algorithms on ESP32"""
    
    # Test data suitable for ESP32
    test_cases = [
    ...
    algorithms = ["ppe", "aes", "parallel_aes", "rsa"]
    crypto_sim = ESP32CryptoSimulator()
    results = {}
    salt = "esp32test"
    iterations = 3  # Low for ESP32 simulation
    ...
    print(f"Algorithms: PPE, AES-128, Parallel AES, RSA-2048")
    ...
    for size_name, test_data in test_cases:
        ...
        for algorithm in algorithms:
            ...
            # Check memory limitations
            try:
                memory_multiplier = 3 if algorithm == "ppe" else (4 if algorithm == "aes" else (5 if algorithm == "parallel_aes" else 10))
            ...
            for iteration in range(iterations):
                try:
                    ...
                    elif algorithm == "parallel_aes":
                        encrypted = crypto_sim.parallel_aes_encrypt(test_data, key)
                    else:
                        encrypted = crypto_sim.ppe_encrypt(test_data, key)
                    ...
                    if algorithm == "rsa":
                        ...
                        memory_usage.append(len(encrypted) + len(test_data) * 5)
                    else:
                    ...
                    if algorithm == "rsa":
                        ...
                        decrypted = crypto_sim.parallel_aes_decrypt(encrypted, key)
                    else:
                        decrypted = crypto_sim.ppe_decrypt(encrypted, key)
...
def plot_algorithm_comparison_charts(results):
    ...
    for size_name, algo_results in results.items():
        ...
        parallel_aes_time = algo_results.get("parallel_aes", {}).get('total_time', 0) if algo_results.get("parallel_aes") else 0
        ...
        parallel_aes_times.append(parallel_aes_time)
        ...
        parallel_aes_mem = algo_results.get("parallel_aes", {}).get('memory_usage', 0) if algo_results.get("parallel_aes") else 0
        ...
        parallel_aes_memory.append(parallel_aes_mem / 1024)
    ...
    ax1.bar(x - width*1.5, ppe_times, width, label='PPE', color=colors['ppe'])
    ax1.bar(x - width/2, aes_times, width, label='AES-128', color=colors['aes'])
    ax1.bar(x + width/2, parallel_aes_times, width, label='Parallel AES', color=colors['parallel_aes'])
    ax1.bar(x + width*1.5, rsa_times, width, label='RSA-2048', color=colors['rsa'])
    ...
    ax2.bar(x - width*1.5, ppe_memory, width, label='PPE', color=colors['ppe'])
    ax2.bar(x - width/2, aes_memory, width, label='AES-128', color=colors['aes'])
    ax2.bar(x + width/2, parallel_aes_memory, width, label='Parallel AES', color=colors['parallel_aes'])
    ax2.bar(x + width*1.5, rsa_memory, width, label='RSA-2048', color=colors['rsa'])
    ...
    if sizes:
        ...
        security_scores = {"ppe": 1, "aes": 3, "parallel_aes": 3, "rsa": 5}
        ...
        for algorithm in algorithms:
            if first_results.get(algorithm):
                ...
                values = [speed_score, memory_score, security_scores[algorithm]]
                values += values[:1]
    ...
    for size_name, algo_results in results.items():
        ...
        for algorithm in algorithms:
            if algo_results.get(algorithm):
                ...
                throughput_labels.append(f"{algorithm_names[algorithm]}\n{size_name}")
    ...
    print("   • Parallel AES shows improved performance for larger data")
...
def print_algorithm_analysis(results):
    ...
    for algorithm in algorithms:
        ...
        if total_times:
            ...
            if algorithm == "ppe":
                ...
                if avg_time < 300:
                    print(f"   ✅ Parallelization shows benefits for larger data")
                else:
                    print(f"   ⚠️  Parallel overhead may be too high for small data")
            elif algorithm == "rsa":
                if avg_time < 2000:
    ...
    print("3. ⚡ Parallel AES: Use for larger data sets where parallelism can help")
    print("4. 🛡️  RSA: Use only for key exchange or small critical data")
    print("5. 📦 Keep data chunks under 1KB for optimal performance")
    print("6. 🧠 Monitor memory usage - RSA uses 10x more memory")
    print("7. ⚖️  Parallel AES shows best results for medium to large data sizes")
    
    print(f"\n🔍 EXPECTED REAL ESP32 PERFORMANCE:")
    print("-" * 50)
    print("• PPE (32-128B): 10-50 ms")
    print("• AES (32-128B): 50-200 ms")  
    print("• Parallel AES (512B-2KB): 100-500 ms")
    print("• RSA (32-128B): 500-2000 ms")
    print("• Memory efficiency: PPE < AES < Parallel AES < RSA")
    print("• Security level: PPE < AES = Parallel AES < RSA")
...
def print_detailed_comparison_table(results):
    ...
    for size_name, algo_results in results.items():
        for algorithm in ["ppe", "aes", "parallel_aes", "rsa"]:
            if algo_results.get(algorithm):
    ...
    print("\n\n📊 Table 3: Memory Efficiency Analysis")
    print("-" * 100)
    print(f"{'Data Size':<20} {'PPE (KB)':<15} {'AES (KB)':<15} {'Parallel AES (KB)':<20} {'RSA (KB)':<15} {'Memory Ratio':<20}")
    print("-" * 100)
    ...
    for size_name, algo_results in results.items():
        ...
        parallel_aes_mem = algo_results.get("parallel_aes", {}).get('memory_usage', 0) / 1024 if algo_results.get("parallel_aes") else 0
        rsa_mem = algo_results.get("rsa", {}).get('memory_usage', 0) / 1024 if algo_results.get("rsa") else 0
        
        if ppe_mem > 0:
            ratio = f"PPE:{ppe_mem:.1f} | AES:{aes_mem:.1f} | PAES:{parallel_aes_mem:.1f} | RSA:{rsa_mem:.1f}"
            print(f"{size_name:<20} {ppe_mem:<15.1f} {aes_mem:<15.1f} {parallel_aes_mem:<20.1f} {rsa_mem:<15.1f} {ratio:<20}")
    ...
    for algorithm in algorithms:
        ...
        if total_times:
            ...
            # Security levels
            security_levels = {"ppe": "Basic", "aes": "High", "parallel_aes": "High", "rsa": "Very High"}
            security_score = {"ppe": 2, "aes": 4, "parallel_aes": 4, "rsa": 5}[algorithm]
...
if __name__ == "__main__":
    ...
    print("This will compare PPE, AES, Parallel AES, and RSA algorithms on ESP32")
    print()
